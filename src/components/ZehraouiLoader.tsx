...
const ZehraouiLoader: React.FC<ZehraouiLoaderProps> = ({ children }) => {
  ...
  // Messages de bienvenue dans différentes langues
  const welcomeMessages = [
    { lang: 'Arabic', text: 'مرحبا' },
    { lang: 'English', text: 'Welcome' },
    { lang: 'French', text: 'Bienvenue' },
  ...
  useEffect(() => {
    // Changement des messages de bienvenue
    const messageTimer = setInterval(() => {
      setCurrentMessageIndex((prev) => (prev + 1) % welcomeMessages.length);
    }, 500);

    // Durée de chargement totale
    const loadingTimer = setTimeout(() => {
      setIsLoading(false);
    }, 3000);
    ...
    return (
      <AnimatePresence>
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={`fixed inset-0 flex flex-col items-center justify-center z-50 space-y-8 transition-colors duration-300 ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white' : 'bg-gradient-to-br from-gray-100 via-gray-50 to-white text-gray-900'}`}
        >
          <motion.div 
            className="flex items-center justify-center space-x-4"
            initial={{ scale: 0.5, opacity: 0 }}
          ...
          >
            {['Z', 'E', 'H', 'R', 'A', 'O', 'U', 'I'].map((letter, index) => (
              <motion.span
                ...
                className={`text-6xl sm:text-8xl md:text-9xl font-extrabold tracking-wide transform hover:rotate-6 transition-transform duration-300 ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}
              >
                {letter}
              </motion.span>
            ))}
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -50 }}
            transition={{ type: 'spring', stiffness: 300 }}
            className="text-4xl font-bold tracking-wide text-center px-4 sm:px-8 md:px-16"
          >
            <p className="text-2xl mb-2 opacity-70">
              {welcomeMessages[currentMessageIndex].lang}
            </p>
          ...
          >
            <motion.div 
              className={`h-full ${theme === 'dark' ? 'bg-blue-600' : 'bg-blue-400'}`}
              initial={{ x: '-100%' }}
              animate={{ x: '0%' }}
              transition={{ duration: 3, ease: 'linear' }}
            />
